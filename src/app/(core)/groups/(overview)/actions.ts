...
export const filterGroupsBySubject = async (
  subject: string
): Promise<Group[]> => {
  ...
  try {
    ...
    switch (subject.toLowerCase()) {
      ...
      case 'social science':
        return data.filter(
          (group) => group.subject.toLowerCase() === 'social science'
        )
      case 'computing':
        return data.filter(
          (group) => group.subject.toLowerCase() === 'computing'
        )
      case 'languages':
        return data.filter(
          (group) => group.subject.toLowerCase() === 'languages'
        )
      case 'all groups':
      default:
